...
func TestSignedRational(t *testing.T) {
	r := SignedRational{-4, 6}
	rn := r.Normalize()
	if rn.Numerator != -2 || rn.Denominator != 3 {
		t.Errorf("Normalization failed, expected: -2/3, actual: %s", rn.ToString())
	}
	if rn.ToString() != "-2/3" {
		t.Errorf("ToString failed, expected: -2/3, actual: %s", rn.ToString())
	}
